...
class ExoPlaybackStateHolder(
     ...
     * [com.decent.usbaudio.media3.UsbAudioSink.setVolume] override when USB DAC bit-perfect mode is
     * active — propagating the volume to the USB DAC hardware via UAC2 SET_CUR on the Feature Unit.
     *
     * Must be called on the main thread (ExoPlayer requirement). The system MediaSession volume
     * provider callback already runs on the main thread, so no extra dispatch is needed for that
     * path.
     *
     * @param linear Linear gain in [0.0, 1.0] (clamped by ExoPlayer internally).
     */
