 ...
 * 1. **Detection layer** — A long-lived, process-wide singleton ([UsbDacConnectionMonitor]) listens
 *    to `UsbManager.ACTION_USB_DEVICE_ATTACHED/DETACHED` broadcasts and re-scans
 *    `UsbManager.deviceList` on every event. It exposes a `StateFlow<Boolean>` so any UI consumer
 *    can react to plug/unplug without owning a `BroadcastReceiver` itself. This is strictly about
 *    *physical presence* — it does not imply the DAC is actively being used.
 * 2. **User-opt-in setting** — USB DAC bit-perfect mode is gated behind a user toggle
 *    (`PlaybackSettings.usbDacMode`, default `false`). The toggle is grayed out in the UI when no
 *    DAC is detected (driven by the connection monitor's StateFlow). This prevents the user from
 *    enabling a mode that would silently do nothing.
 * 3. **Steady-state fallback** — When the user has enabled USB DAC mode but no DAC is connected (or
 *    the DAC was unplugged mid-playback), audio must continue without interruption. The player
 *    transparently falls back to Android's default `AudioFlinger` sink. The UI overlay must NOT
 *    label this state as "Pending" or "Connecting" — those words imply a transient state that will
 *    resolve on its own. The correct label is one that describes the actual steady-state path: e.g.
 *    `Android MediaCodec (codec)` for the decoder and `Android (AudioFlinger)` for the engine. See
 *    the `deriveDecoderInfo` function below.
 ...
 *    pipeline is not actually active (no alive stream AND no running native engine), which is the
 *    signal to the UI layer to render the fallback path. Returning `null` rather than a partial
 *    snapshot prevents the UI from showing misleading fields like "FFmpeg (float → int)" when
 *    FFmpeg is not actually feeding any USB stream.
 * 5. **Bit-perfect honesty** — The overlay shows `✓` for true bit-perfect passthrough only when the
 *    sink reports `Passthrough` or `Bit-perfect` / `PCM (lossless)`. Any other path (including the
 *    steady-state fallback) shows `✗`. This is critical for users who care about bit-perfect output
 *    — they must be able to trust the overlay at a glance.
 * 6. **Persistence on unplug** — Per user spec, the overlay stays visible when a DAC is unplugged
 *    mid-playback. The polling loop continues to run, the overlay shows the fallback path, and the
 *    user can replug the DAC to resume bit-perfect mode without losing their playback position. The
 *    overlay must not auto-hide or show an error state.
 */
data class AudioInfo(
    ...
    /**
     * Whether a USB Audio Class DAC is currently physically connected to the device, regardless of
     * whether USB DAC mode is enabled. Driven by [UsbDacConnectionMonitor]. The UI uses this to
     * distinguish three connection states that have distinct user-meaningful labels:
    ...
    companion object {
         ...
         * @param usbDacConnected Whether a USB Audio Class DAC is currently physically connected to
         *   the device, per [UsbDacConnectionMonitor]. Used to label the steady-state fallback path
         *   accurately: when mode is on but no DAC is connected, the overlay must NOT show
        ...
        fun from(
            ...
            if (snapshot == null) {
                ...
                // below reflects the actual decoder (ExoPlayer's MediaCodec) — see
                // deriveDecoderInfo.
                return AudioInfo(
         ...
         * MediaCodec — there is no transient "Pending" decoder state. The previous label "Pending
         * (codec)" was misleading because it implied the decoder was about to switch to a
         ...
         *
         * The distinction between "mode off" and "mode on but no DAC" is now communicated via the
         * `resamplerStatus` field ("Android resampler" vs "Android mixer (no DAC)") rather than via
         * a misleading decoder label.
         */
        @Suppress(
            "UNUSED_PARAMETER"
        ) // usbDacModeActive kept for API symmetry + future per-mode decoder selection
